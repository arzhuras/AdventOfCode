...
def getNextElt2(lstStr, idx):  # forcément une liste avec [ & ]
    # print("getNextElt", lstStr[idx:], idx)
    if idx >= len(lstStr) - 1 or len(lstStr) == 2:  # liste terminée
        # print("  getNextElement: no more element")
        return -1, None
    ...
    startIdx = idx
    idx += 1
    while lstStr[idx] != "," and lstStr[idx] != "]":
        idx += 1

    elt = lstStr[startIdx:idx]
    idx += 1
...
def parseNestedList(nestedList, tab=""):
    # print(f"{tab}{nestedList}")
    lst = []
    idx = 0
    while True:
        idx, elt = getNextElt2(nestedList, idx)
        # print(f"{tab}  '{elt}' ({idx}) ")
        if elt == None:
            break
        if elt[0] == "[":
            lst.append(parseNestedList(elt, tab + "  "))
        else:
            lst.append(elt)
    return lst
...
# test de parsing nestedList generique
for line in data.rawInput:
    if len(line) > 0:
        print("parse:", line)
        lst = parseNestedList(line)
        print("->", lst)
        print()
