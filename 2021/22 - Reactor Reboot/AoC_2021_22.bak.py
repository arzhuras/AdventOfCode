...
def explode(region, intersect):  # region = LIGTH, XLEFT, XRIGHT, YUP, YDOWN, ZFRONT, ZREAR
    ...
    if region[5] != intersect[5]:  # encadrement avant sur les Z
        exploded.append(
            [region[0], region[1], region[2], region[3], region[4], region[5], intersect[5] - 1, str(region) + "FRONT"]
        )
        # print("  FRONT", exploded[-1])

    if region[6] != intersect[6]:  # encadrement arriere sur les Z
        exploded.append(
            [region[0], region[1], region[2], region[3], region[4], intersect[6] + 1, region[6], str(region) + "REAR"]
        )
...
def resolve_part2():
    ...
    print(ANSI_YELLOW, "line:", 1, line[0], regionSize(line[0]), ANSI_NORM)

    for i in range(1, len(line)):
        print(ANSI_YELLOW, "line:", i + 1, line[i], regionSize(line[i]), ANSI_NORM)
        # if i == 11:
        # exit()
        curRegion = line[i]
        # while len(curRegion) > 0:
        print("  curRegion:", curRegion)
        print("  regionLst:", len(regionLst), regionLst)
        print("  explodedLst:", len(curRegion), curRegion)
        for idx, region in enumerate(regionLst):
            print(idx + 1, len(regionLst), region)
            resIntersect = intersect(region, curRegion)
            ...
            resExplode = explode(curRegion, resIntersect)
            if len(resExplode) == 0:
                resExplode = [curRegion]

            # add newly exploded regions to the scan list
            if region[0] != resIntersect[0]:  # if light is different
                # if region[0] != resIntersect[0]:  # if light is different
                print(ANSI_RED, "    keep intersect", resIntersect, ANSI_NORM)
                regionLst.append(resIntersect)
                # explodedLst.append(resIntersect)
            else:
                print(ANSI_PURPLE, "    keep extrasect", len(resExplode), ANSI_NORM)
                for tmpRegion in resExplode:
                    regionLst.append(tmpRegion)

            break  # check newly created region against all existing region

        if len(resIntersect) == 0:
            regionLst.append(curRegion)
            print(ANSI_GREY, "regionLst updated", regionLst[-1], regionSize(curRegion), ANSI_NORM)
    ...
    for region in regionLst:
        if region[0] == True:
            ...
            print("ADD", region, regionSize(region), sum)
        else:
            sum -= regionSize(region)
            print("SUB", region, regionSize(region), sum)
